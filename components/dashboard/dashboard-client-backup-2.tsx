...
export function DashboardClient() {
  ...
  const [licenseStatus, setLicenseStatus] = useState<{ valida: boolean; diasRestantes: number } | null>(null);
  const [showWarningPopup, setShowWarningPopup] = useState(false);
  const [showErrorPopup, setShowErrorPopup] = useState<{ show: boolean, message: string }>({ show: false, message: '' });

  const handleLicenseResponse = (data: any, newToken: string) => {
    if (data.error || !data.valida) {
      setShowErrorPopup({ show: true, message: data.error || 'Sua licença de uso expirou ou foi inativada. Entre em contato para regularizar o acesso.' });
      handleLogout();
      return;
    }
    
    setLicenseStatus(data);
    localStorage.setItem('apollo_dashboard_token', newToken);
    setToken(newToken);
    
    if (data.diasRestantes <= 5) {
      const today = getTodayStr();
      const lastWarning = localStorage.getItem('apollo_warning_date');
  ...
  // Carregar token do localStorage no início e verificar licença
  useEffect(() => {
    setMounted(true);
    setSelectedDate(getTodayStr());
    const savedToken = localStorage.getItem('apollo_dashboard_token');
    if (savedToken) {
      fetch(`/api/license?chave=${savedToken}`)
        .then(res => res.json())
        .then(data => handleLicenseResponse(data, savedToken))
        .catch(() => setLicenseStatus({ valida: true, diasRestantes: 999 }));
    } else {
      // Se não tem token (modo apresentação), não precisa validar licença
      setLicenseStatus({ valida: true, diasRestantes: 999 });
    }
  }, []);
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 401 || res.status === 403) {
          throw new Error(errData.error || 'Token/Chave inválida ou expirada');
        }
  ...
  const handleLogin = () => {
    ...
    const newToken = tempToken.trim();
    setShowTokenInput(false);
    setTempToken('');

    fetch(`/api/license?chave=${newToken}`)
      .then(res => res.json())
      .then(data => handleLicenseResponse(data, newToken))
      .catch(() => setShowErrorPopup({ show: true, message: 'Erro de conexão ao validar a licença.' }));
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      {showWarningPopup && licenseStatus && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-[#1a1d24] border border-orange-500/50 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl relative">
            <div className="absolute top-0 left-0 w-full h-1 bg-orange-500" />
            <div className="p-8 text-center">
              <div className="w-16 h-16 bg-orange-500/10 rounded-full flex items-center justify-center mx-auto mb-6">
                <AlertCircle className="w-8 h-8 text-orange-500" />
              </div>
              <h2 className="text-2xl font-bold mb-3 text-white">Atenção ao Vencimento</h2>
              <p className="text-gray-300 mb-8 leading-relaxed">
                A sua licença de uso do painel expira em <strong className="text-orange-400 text-lg">{licenseStatus.diasRestantes} dia{licenseStatus.diasRestantes === 1 ? '' : 's'}</strong>.<br/>
                Entre em contato com o administrador para renovar e evitar interrupções.
              </p>
              <button 
                onClick={() => {
                  localStorage.setItem('apollo_warning_date', getTodayStr());
                  setShowWarningPopup(false);
                }}
                className="w-full py-3.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl font-bold transition-colors text-lg"
              >
                Ciente
              </button>
            </div>
      ...
      {/* Popup de Erro/Bloqueio (Substitui o alert nativo) */}
      {showErrorPopup.show && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-[100] p-4">
          <div className="bg-[#1a1d24] border border-red-500/50 rounded-2xl w-full max-w-md overflow-hidden shadow-2xl relative animate-in zoom-in-95 duration-200">
            <div className="absolute top-0 left-0 w-full h-1 bg-red-500" />
            <div className="p-8 text-center">
              <div className="w-16 h-16 bg-red-500/10 rounded-full flex items-center justify-center mx-auto mb-6">
                <Lock className="w-8 h-8 text-red-500" />
              </div>
              <h2 className="text-2xl font-bold mb-3 text-white">Acesso Negado</h2>
              <p className="text-gray-300 mb-8 leading-relaxed">
                {showErrorPopup.message}
              </p>
              <button 
                onClick={() => setShowErrorPopup({ show: false, message: '' })}
                className="w-full py-3.5 bg-red-500 hover:bg-red-600 text-white rounded-xl font-bold transition-colors text-lg"
              >
                Entendi
              </button>
            </div>
